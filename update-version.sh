...
if git diff --exit-code VERSION > /dev/null; then
    ...
    # push the commit
    git push -u "https://${GITHUB_ACTOR}:${GITHUB_TOKEN}@github.com/aws/aws-iot-device-sdk-js-v2.git" ${version_branch}
    ...
    # delete the old tag
    git tag -d ${version}
    git push "https://${GITHUB_ACTOR}:${GITHUB_TOKEN}@github.com/aws/aws-iot-device-sdk-js-v2.git" :refs/tags/${version}
