...
This sample functions exactly like the [Node PubSub sample](../pub_sub/README.md), but it uses pure Javascript rather than Typescript to consume and run the SDK. If you cannot use Typescript, do not want type-annotations, or otherwise prefer to use pure Javascript, then this sample can serve as a basis for starting your application.
...
To Run this sample, go to the `node/pub_sub_js` folder and run the following commands:
...
``` sh
npm install
node dist/index.js --endpoint <endpoint> --cert <file> --key <file> --ca_file <file>
```
