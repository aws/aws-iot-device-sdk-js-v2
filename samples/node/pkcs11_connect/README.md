...
To run the PKCS#11 connect sample, go to the `node/pkcs11_connect` folder and run the following commands:

```sh
npm install
node dist/index.js --endpoint <endpoint> --cert <path to certificate> --pkcs11_lib <libsofthsm2.so> --pin <user-pin> --token_label <token-label> --key_label <key-label>
```
...
```sh
npm install
node dist/index.js --endpoint <endpoint> --cert <path to certificate> --pkcs11_lib <libsofthsm2.so> --pin <user-pin> --token_label <token-label> --key_label <key-label> --ca_file <path to CA file>
```
...
6. Now you can run the sample with the following from the `node/pkcs11_connect` folder:
